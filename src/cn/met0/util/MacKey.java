...
public class MacKey {
	...
	public String Str3MAC(String strKey, String strInitData, String strMacData) throws Exception{
    ...
    { // 一个字节的数，
        ...
        {
            // 整数转成十六进制表示
            stmp = java.lang.Integer.toHexString(b[n] & 0XFF);// (java.lang.Integer.toHexString(b[n] & 0XFF));
            if (stmp.length() == 1)
                hs = hs + "0" + stmp;
            else
                hs = hs + stmp;
	...
	private static byte[] hex2byte(byte[] b) throws Exception
    {
        ...
        {
            byte[] bs = new byte[2];
            bs=new byte[]{b[n],b[n+1]};
            String item = new String(bs);
